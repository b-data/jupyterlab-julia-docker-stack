...
if [ $(id -u) == 0 ] ; then
    ...
    if [[ $(find .local/share/code-server/extensions -type d -name "julialang.language-julia-1.0.8" | wc -l) != "0" ]]; then
      su $NB_USER -c "rm -r .local/share/code-server/extensions/julialang.language-julia-1.0.8"
    fi
    if [[ ! -d ".local/share/code-server/extensions/julialang.language-julia-1.0.10.vsix" ]]; then
      su $NB_USER -c "code-server --install-extension /var/tmp/julialang.language-julia-1.0.10.vsix || true"
    fi
    ...
    # Install Code Server extension for Julia Language Supprt
    if [[ $(find .local/share/code-server/extensions -type d -name "julialang.language-julia-1.0.8" | wc -l) != "0" ]]; then
      rm -r .local/share/code-server/extensions/julialang.language-julia-1.0.8
    fi
    if [[ ! -d ".local/share/code-server/extensions/julialang.language-julia-1.0.10.vsix" ]]; then
      code-server --install-extension /var/tmp/julialang.language-julia-1.0.10.vsix || true
    fi
