...
RUN curl -sLO https://bootstrap.pypa.io/get-pip.py \
  ...
  && curl -sLO https://dl.b-data.ch/vsix/GitLab.gitlab-workflow-3.2.0.vsix \
  && code-server --extensions-dir ${CODE_BUILTIN_EXTENSIONS_DIR} --install-extension GitLab.gitlab-workflow-3.2.0.vsix \
