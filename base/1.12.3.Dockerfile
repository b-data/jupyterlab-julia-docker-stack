...
ARG JULIA_CUDA_PACKAGE_VERSION=5.9.6
...
RUN export JULIA_DEPOT_PATH=${JULIA_PATH}/local/share/julia \
  ...
  && code-server --extensions-dir ${CODE_BUILTIN_EXTENSIONS_DIR} --install-extension julialang.language-julia@1.158.2 \
  ## Clean up
  && rm -rf /tmp/* \
    /var/lib/apt/lists/* \
    ${HOME}/.cache \
